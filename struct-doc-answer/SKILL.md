...
## 二、文档结构（flomo 格式，对应 news-docs 4 章节）

news-docs 4 章节（`## 来源` `## 概述` `## 核心概念` `## 分类标签`）升级到 flomo 格式的对应关系：
...
第一句精确定义。第二句背景与现实意义。第三句核心逻辑、演变脉络或关键争议。
...
- `## 概述` 和 `## 核心概念` 内容直接写，**不**加 `##` 标题（flomo 不支持），也不强制段落名
- 子概念可以用 `### 子概念名` 形式（flomo 实际可能渲染为加粗）
...
2. **来源行**（`**来源**：xxx、xxx`）
3. **概述**（`## 概述` 内容直接写）— 第一句精确定义，第二句背景，第三句核心逻辑
4. **核心概念**（`## 核心概念` 内容直接写，可选 `### 子概念一` `### 子概念二` 分节）
5. **可选列表**（用 `-` 或 `1.` 列举要点）
