...
- 必填输出：标题、来源、字符数、正文预览（最少 600 字符）
- 字符数为 0 时立刻停手报告

#### 第二步：构造 ai-content

按 SKILL §2 文档结构构造 `--ai-content`：
- 必须有 `**概念**` 段（含 `<mark>核心概念</mark>` 高亮）
- 必须有 `**子概念**` 段（≥6 个 `<mark>要点</mark>：论点+引用原文关键数据`）
- 完整长 ai-content，禁止短占位符测试
...
脚本会自动完成：
- 抓取（uploads 步骤内部）
- 关键词分类确定 domain/subdomain
- 创建本地文件
- pre-commit hook 验证格式
- search_flomo 查重
- **relevance ≥ 0.9 时自动 fetch_flomo_memo 拉旧文档到 stderr**（commit `266138f`）
- AI 据脚本输出（旧文档 + 新内容）决策
- 上传到 flomo
...
#### relevance 决策自动化（脚本默认行为）

脚本默认行为（用户无需显式决定）：
- **relevance < 0.9** → 脚本自动 continue 新建（AI 不主动插手）
- **relevance ≥ 0.9** → 脚本打印决策表 + 强制 AI 显式选 `--force-new` / `--update MEMO_ID` / skip

AI 拿到的脚本 stderr 输出会含：
- "检测到相似笔记 id={OLD_ID}（relevance={X}）"
- "========== 已有笔记内容（id={OLD_ID}, N 字符） ==========" + 旧 markdown 全文
- "========== 新文章内容（N 字符） ==========" + 新 body_text 全文
- "relevance >= 0.9 决策表" + 强制规则

AI 必须根据脚本 stderr 输出的"旧文档已就绪"段判断，然后传给 `--update MEMO_ID` 或 `--force-new`。
...
每条微信公众号 URL 必须严格按以下三步顺序执行：

```
Step 1: fetch_wechat_article(use_cache=False) → 拿标题/来源/字符数/正文预览
Step 2: 构造完整长 ai-content（必须含概念段 + ≥6 子概念 + mark 高亮）
Step 3: process_inbox.py --url ... --ai-content "<完整合并 markdown>"
```

**禁止**：(a) 跳过 Step1 直接跑 process_inbox（拿到的是缓存过时内容）
