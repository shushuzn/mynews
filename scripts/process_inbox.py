            ...
            if _os_for_tty.isatty(0):
                ...
                if relevance >= 0.9:
                    ...
                    if getattr(args, 'force_new', False):
                        ...
                        print(f"  [flomo] 未上传。检测到高相似笔记 id={old_id}（relevance={relevance:.2f}），已自动跳过。")
                        print(f"  [flomo] 请比对上方旧笔记与新内容，判断是否实质相同：")
                        print(f"  [flomo]   - 完全相同 + 零增量 → 跳过（不动）")
                        print(f"  [flomo]   - 完全相同 + 有增量 → 重跑: --update {old_id}")
